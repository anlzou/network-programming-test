...
void CBitButtonNL::DrawTheBitmap(CDC* pDC,  RECT* rpItem, 
								  BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsDisabled && (byIndex == 0) )
	{
		...
		hBitmap = ::CreateCompatibleBitmap(pDC->m_hDC, m_csBitmaps[byIndex].dwWidth,
		...
		rRect.left = 0;
		rRect.top = 0;
		rRect.right = rImage.right + 1;			//右边框默认不被包括
		rRect.bottom = rImage.bottom + 1;		//底部边框同上，所以要加1
		::FillRect(hDC, &rRect, (HBRUSH)RGB(255, 255, 255));	//将矩形范围内全部填充
		...
		::BitBlt(hDC, 0, 0, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight,
					hdcBmpMem, 0, 0, SRCCOPY);
		
		::SetBkColor(hDC, crOldColor);		//恢复设备原来的背景色
		::SelectObject(hDC, hOldBmp2);		//恢复原来的位图
		::DeleteDC(hDC);					//删除设备
	...
	{
		::BitBlt(pDC->m_hDC,rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth,
				 m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCCOPY);
	} // else
	
	::SelectObject(hdcBmpMem, hbmOldBmp);
	::DeleteObject(hbmOldBmp);
	::DeleteDC(hdcBmpMem);
} // End of DrawTheBitmap

void CBitButtonNL::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem,
							   CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	// Draw the button's title
	// If button is pressed then "press" title also
	if (bIsPressed)
		rpCaption->OffsetRect(m_ptPressedOffset.x, m_ptPressedOffset.y);	//偏移矩形
	...
	//format text in the given rectangle
	pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_CENTER | DT_CALCRECT);
	
	switch(m_byTextAlign)
	{
	case NL_ALIGN_LEFT:
		rpCaption->OffsetRect((centerRect.Width() - rpCaption->Width()) / 5,
							  (centerRect.Height() - rpCaption->Height()) / 2 - 1);
		break;
	case NL_ALIGN_RIGHT:
		rpCaption->OffsetRect((centerRect.Width() - rpCaption->Width()) * 4 / 5,
							  (centerRect.Height() - rpCaption->Height()) / 2 - 1);
		break;
	...
	if (bIsDisabled && m_bDrawDisableText)
	{
		//绘制出3D的效果
...
BOOL CBitButtonNL::SetForceColor(COLORREF crColorNormal, COLORREF crColorMouseDown,
								 COLORREF crColorHigh, BOOL bRepaint)
{
	// Set new color
...
void CBitButtonNL::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (m_csBitmaps[0].hBitmap)
	{
		...
		DrawTheBitmap(pDC,&lpDrawItemStruct->rcItem,
			 m_bIsPressed, m_bIsDisabled);
	} // if
	
	// Write the button title (if any)
	if (!sTitle.IsEmpty() && m_bDrawText) 
	{
		DrawTheText(pDC, (LPCTSTR)sTitle, &lpDrawItemStruct->rcItem, 
...
void CBitButtonNL::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	wndUnderMouse = WindowFromPoint(point);		//获得指定点窗口句柄
...
void CBitButtonNL::SizeToContent(void)
{
	if (m_csBitmaps[0].hBitmap)
	{
		...
		SetWindowPos(	NULL, 0, 0, m_csBitmaps[0].dwWidth, m_csBitmaps[0].dwHeight,
...
BOOL CBitButtonNL::SetBtnCursor(int nCursorId, BOOL bRepaint)
{
	...
	if (nCursorId)
	{
		...
		m_hCursor = (HCURSOR)::LoadImage(hInstResource, MAKEINTRESOURCE(nCursorId),
										 IMAGE_CURSOR, 0, 0, 0);
		if (bRepaint)
			Invalidate();
		if (m_hCursor == NULL)
...
CBitButtonNL& CBitButtonNL::SetFontName(const CString& strFont, BYTE byCharSet , BOOL bRepaint)
{	
